...
13. <span data-ttu-id="2d7a8-194">Abra el archivo de clase de **programa** y compruebe que el código es el mismo que el siguiente:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-194">Open the **Program** class file and verify the code in it is the same as the following:</span></span>
...
5.  <span data-ttu-id="2d7a8-272">Mientras sigue en el **archivo > la configuración de compilación** y asegúrese de que:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-272">While still in **File > Build Settings** and make sure that:</span></span>
    ...
    6.  <span data-ttu-id="2d7a8-279">Guarde la escena y agréguela a la compilación.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-279">Save the scene and add it to the build.</span></span> 
        ...
        3. <span data-ttu-id="2d7a8-285">Abra la carpeta **Scenes** recién creada y, a continuación, en el campo *nombre de archivo* :, escriba **BotScene** y, a continuación, haga clic en **Guardar** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-285">Open your newly created **Scenes** folder, and then in the *File name* : text field, type **BotScene** , then click on **Save** .</span></span>
...
## <a name="chapter-8--create-the-botobjects-class"></a><span data-ttu-id="2d7a8-346">Capítulo 8: creación de la clase BotObjects</span><span class="sxs-lookup"><span data-stu-id="2d7a8-346">Chapter 8 – Create the BotObjects class</span></span>
...
## <a name="chapter-9--create-the-gazeinput-class"></a><span data-ttu-id="2d7a8-359">Capítulo 9: creación de la clase GazeInput</span><span class="sxs-lookup"><span data-stu-id="2d7a8-359">Chapter 9 – Create the GazeInput class</span></span>
...
14.  <span data-ttu-id="2d7a8-405">Asegúrese de guardar los cambios en *Visual Studio* antes de volver a *Unity* .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-405">Be sure to save your changes in *Visual Studio* before returning to *Unity* .</span></span>

## <a name="chapter-11--create-the-interactions-class"></a><span data-ttu-id="2d7a8-406">Capítulo 11: crear la clase Interactions</span><span class="sxs-lookup"><span data-stu-id="2d7a8-406">Chapter 11 – Create the Interactions class</span></span>

<span data-ttu-id="2d7a8-407">La clase que va a crear ahora se denomina **interacciones** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-407">The class you are going to create now is called **Interactions** .</span></span> <span data-ttu-id="2d7a8-408">Esta clase se usa para detectar la entrada de la pulsación de HoloLens del usuario.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-408">This class is used to detect the HoloLens Tap Input from the user.</span></span> 

<span data-ttu-id="2d7a8-409">Si el usuario pulsa mientras mira el objeto *Bot* en la escena y el bot está listo para escuchar las entradas de voz, el objeto bot cambiará el color a **rojo** y comenzará a escuchar las entradas de voz.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-409">If the user taps while looking at the *Bot* object in the scene, and the Bot is ready to listen for voice inputs, the Bot object will change color to **red** and begin listening for voice inputs.</span></span> 

<span data-ttu-id="2d7a8-410">Esta clase hereda de la clase **GazeInput** y, por lo tanto, puede hacer referencia al método **Start ()** y a las variables de esa clase, que se indican mediante el uso de **base** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-410">This class inherits from the **GazeInput** class, and so is able to reference the **Start()** method and variables from that class, denoted by the use of **base** .</span></span> 
 
<span data-ttu-id="2d7a8-411">Para crear esta clase:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-411">To create this class:</span></span>

1.  <span data-ttu-id="2d7a8-412">Haga doble clic en la carpeta **scripts** para abrirla.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-412">Double-click on the **Scripts** folder, to open it.</span></span> 
2.  <span data-ttu-id="2d7a8-413">Haga clic con el botón derecho en la carpeta **scripts** y haga clic en **crear > script de C#** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-413">Right-click inside the **Scripts** folder, click **Create > C# Script** .</span></span> <span data-ttu-id="2d7a8-414">Asigne un nombre a las **interacciones** del script.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-414">Name the script **Interactions** .</span></span> 
3.  <span data-ttu-id="2d7a8-415">Haga doble clic en el nuevo script para abrirlo con Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-415">Double-click on the new script to open it with Visual Studio.</span></span>
4.  <span data-ttu-id="2d7a8-416">Actualice los espacios de nombres y la herencia de clases para que sea igual que la siguiente, en la parte superior de la clase **Interactions** :</span><span class="sxs-lookup"><span data-stu-id="2d7a8-416">Update the namespaces and the class inheritance to be the same as the following, at the top of the **Interactions** class:</span></span>
...
5.  <span data-ttu-id="2d7a8-417">Dentro de la clase **Interactions** , agregue la siguiente variable:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-417">Inside the **Interactions** class add the following variable:</span></span>
    ...
    ```
6.  <span data-ttu-id="2d7a8-418">A continuación, agregue el método **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="2d7a8-418">Then add the **Start()** method:</span></span>
...
7.  <span data-ttu-id="2d7a8-419">Agregar el controlador que se desencadenará cuando el usuario realice el gesto de TAP delante de la cámara HoloLens</span><span class="sxs-lookup"><span data-stu-id="2d7a8-419">Add the handler that will be triggered when the user performs the tap gesture in front of the HoloLens camera</span></span>
...
8. <span data-ttu-id="2d7a8-420">Asegúrese de guardar los cambios en *Visual Studio* antes de volver a *Unity* .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-420">Be sure to save your changes in *Visual Studio* before returning to *Unity* .</span></span>

## <a name="chapter-12--create-the-sceneorganiser-class"></a><span data-ttu-id="2d7a8-421">Capítulo 12: creación de la clase SceneOrganiser</span><span class="sxs-lookup"><span data-stu-id="2d7a8-421">Chapter 12 – Create the SceneOrganiser class</span></span>

<span data-ttu-id="2d7a8-422">La última clase requerida en este laboratorio se denomina **SceneOrganiser** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-422">The last class required in this Lab is called **SceneOrganiser** .</span></span> <span data-ttu-id="2d7a8-423">Esta clase configurará la escena mediante programación, agregando componentes y scripts a la cámara principal y creando los objetos adecuados en la escena.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-423">This class will setup the scene programmatically, by adding components and scripts to the Main Camera, and creating the appropriate objects in the scene.</span></span>
 
<span data-ttu-id="2d7a8-424">Para crear esta clase:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-424">To create this class:</span></span>

1.  <span data-ttu-id="2d7a8-425">Haga doble clic en la carpeta **scripts** para abrirla.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-425">Double-click on the **Scripts** folder, to open it.</span></span> 
2.  <span data-ttu-id="2d7a8-426">Haga clic con el botón derecho en la carpeta **scripts** y haga clic en **crear > script de C#** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-426">Right-click inside the **Scripts** folder, click **Create > C# Script** .</span></span> <span data-ttu-id="2d7a8-427">Asigne al script el nombre **SceneOrganiser** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-427">Name the script **SceneOrganiser** .</span></span> 
3.  <span data-ttu-id="2d7a8-428">Haga doble clic en el nuevo script para abrirlo con Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-428">Double-click on the new script to open it with Visual Studio.</span></span>
4.  <span data-ttu-id="2d7a8-429">Dentro de la clase **SceneOrganiser** , agregue las siguientes variables:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-429">Inside the **SceneOrganiser** class add the following variables:</span></span>
...
6.  <span data-ttu-id="2d7a8-430">A continuación, agregue los métodos **activo ()** e **Inicio ()** :</span><span class="sxs-lookup"><span data-stu-id="2d7a8-430">Then add the **Awake()** and **Start()** methods:</span></span>
...
7.  <span data-ttu-id="2d7a8-431">Agregue el método siguiente, responsable de crear el objeto bot en la escena y configurar los parámetros y componentes:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-431">Add the following method, responsible for creating the Bot object in the scene and setting up the parameters and components:</span></span>
...
8.  <span data-ttu-id="2d7a8-433">Asegúrese de guardar los cambios en *Visual Studio* antes de volver a *Unity* .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-433">Be sure to save your changes in *Visual Studio* before returning to *Unity* .</span></span>
9.  <span data-ttu-id="2d7a8-434">En el editor de Unity, arrastre el script **SceneOrganiser** desde la carpeta scripts a la cámara principal.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-434">In the Unity Editor, drag the **SceneOrganiser** script from the Scripts folder to the Main Camera.</span></span> <span data-ttu-id="2d7a8-435">El componente de organizador de escenas debe aparecer ahora en el objeto de cámara principal, como se muestra en la imagen siguiente.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-435">The Scene Organiser component should now appear on the Main Camera object, as shown in the image below.</span></span>

    ![Cree el Azure Bot Service](images/AzureLabs-Lab312-37.png)

## <a name="chapter-13--before-building"></a><span data-ttu-id="2d7a8-437">Capítulo 13: antes de compilar</span><span class="sxs-lookup"><span data-stu-id="2d7a8-437">Chapter 13 – Before building</span></span>

<span data-ttu-id="2d7a8-438">Para realizar una prueba exhaustiva de la aplicación, debe transferirla a su HoloLens.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-438">To perform a thorough test of your application you will need to sideload it onto your HoloLens.</span></span>
<span data-ttu-id="2d7a8-439">Antes de hacerlo, asegúrese de que:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-439">Before you do, ensure that:</span></span>

-   <span data-ttu-id="2d7a8-440">Toda la configuración mencionada en el [**capítulo 4**](#chapter-4--set-up-the-unity-project) se establece correctamente.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-440">All the settings mentioned in the [**Chapter 4**](#chapter-4--set-up-the-unity-project) are set correctly.</span></span> 
-   <span data-ttu-id="2d7a8-441">El script **SceneOrganiser** se adjunta al objeto de **cámara principal** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-441">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span> 
-   <span data-ttu-id="2d7a8-442">En la clase **Bot** , asegúrese de que ha insertado la **clave de bot secreta** en la variable **botSecret** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-442">In the **Bot** class, make sure you have inserted your **Bot Secret Key** into the **botSecret** variable.</span></span>

## <a name="chapter-14--build-and-sideload-to-the-hololens"></a><span data-ttu-id="2d7a8-443">Capítulo 14: compilar y transferir localmente a HoloLens</span><span class="sxs-lookup"><span data-stu-id="2d7a8-443">Chapter 14 – Build and Sideload to the HoloLens</span></span>

<span data-ttu-id="2d7a8-444">Ya se ha completado todo lo necesario para la sección Unity de este proyecto, por lo que es el momento de compilarla desde Unity.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-444">Everything needed for the Unity section of this project has now been completed, so it is time to build it from Unity.</span></span>

1.  <span data-ttu-id="2d7a8-445">Vaya a **configuración de compilación** , **archivo > configuración de compilación..** ..</span><span class="sxs-lookup"><span data-stu-id="2d7a8-445">Navigate to **Build Settings** , **File > Build Settings…** .</span></span>
2.  <span data-ttu-id="2d7a8-446">En la ventana **configuración de compilación** , haga clic en **compilar** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-446">From the **Build Settings** window, click **Build** .</span></span>

    ![Compilar la aplicación desde Unity](images/AzureLabs-Lab312-38.png)

3.  <span data-ttu-id="2d7a8-448">Si aún no lo está, marque los **proyectos de C# de Unity** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-448">If not already, tick **Unity C# Projects** .</span></span>
4.  <span data-ttu-id="2d7a8-449">Haga clic en **Generar** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-449">Click **Build** .</span></span> <span data-ttu-id="2d7a8-450">Unity iniciará una ventana del **Explorador de archivos** , donde tendrá que crear y seleccionar una carpeta en la que compilar la aplicación.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-450">Unity will launch a **File Explorer** window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="2d7a8-451">Cree esa carpeta ahora y asígnele el nombre **App** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-451">Create that folder now, and name it **App** .</span></span> <span data-ttu-id="2d7a8-452">Después, con la carpeta de la **aplicación** seleccionada, haga clic en **Seleccionar carpeta** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-452">Then with the **App** folder selected, click **Select Folder** .</span></span> 
5.  <span data-ttu-id="2d7a8-453">Unity comenzará a compilar el proyecto en la carpeta de la **aplicación** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-453">Unity will begin building your project to the **App** folder.</span></span> 
6.  <span data-ttu-id="2d7a8-454">Una vez que Unity termine de compilar (puede tardar algún tiempo), se abrirá una ventana del **Explorador de archivos** en la ubicación de la compilación (Compruebe la barra de tareas, ya que es posible que no aparezca siempre por encima de las ventanas, pero le notificará la adición de una nueva ventana).</span><span class="sxs-lookup"><span data-stu-id="2d7a8-454">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-15--deploy-to-hololens"></a><span data-ttu-id="2d7a8-455">Capítulo 15: implementación en HoloLens</span><span class="sxs-lookup"><span data-stu-id="2d7a8-455">Chapter 15 – Deploy to HoloLens</span></span>

<span data-ttu-id="2d7a8-456">Para implementar en HoloLens:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-456">To deploy on HoloLens:</span></span>

1.  <span data-ttu-id="2d7a8-457">Necesitará la dirección IP de HoloLens (para la implementación remota) y para asegurarse de que HoloLens está en **modo de desarrollador** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-457">You will need the IP Address of your HoloLens (for Remote Deploy), and to ensure your HoloLens is in **Developer Mode** .</span></span> <span data-ttu-id="2d7a8-458">Para ello, siga estos pasos:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-458">To do this:</span></span>

    1. <span data-ttu-id="2d7a8-459">Mientras se contenga HoloLens, abra la **configuración** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-459">Whilst wearing your HoloLens, open the **Settings** .</span></span>
    2. <span data-ttu-id="2d7a8-460">Vaya a **Network & Internet > Wi-Fi > opciones avanzadas** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-460">Go to **Network & Internet > Wi-Fi > Advanced Options**</span></span>
    3. <span data-ttu-id="2d7a8-461">Anote la dirección **IPv4** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-461">Note the **IPv4** address.</span></span>
    4. <span data-ttu-id="2d7a8-462">A continuación, vuelva a **configuración** y, a continuación, **actualice & seguridad > para desarrolladores**</span><span class="sxs-lookup"><span data-stu-id="2d7a8-462">Next, navigate back to **Settings** , and then to **Update & Security > For Developers**</span></span> 
    5. <span data-ttu-id="2d7a8-463">Establezca el modo de Desarrollador en.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-463">Set Developer Mode On.</span></span>

2.  <span data-ttu-id="2d7a8-464">Vaya a la nueva compilación de Unity (la carpeta de la **aplicación** ) y abra el archivo de solución con **Visual Studio** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-464">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio** .</span></span>
3.  <span data-ttu-id="2d7a8-465">En la **configuración de soluciones** , seleccione **depurar** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-465">In the **Solution Configuration** select **Debug** .</span></span>
4.  <span data-ttu-id="2d7a8-466">En la **plataforma** de la solución, seleccione **x86** , **equipo remoto** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-466">In the **Solution Platform** , select **x86** , **Remote Machine** .</span></span> 

    ![Implemente la solución desde Visual Studio.](images/AzureLabs-Lab312-39.png)
 
5.  <span data-ttu-id="2d7a8-468">Vaya al **menú compilar** y haga clic en **implementar solución** para transferir localmente la aplicación a HoloLens.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-468">Go to the **Build menu** and click on **Deploy Solution** , to sideload the application to your HoloLens.</span></span>
6.  <span data-ttu-id="2d7a8-469">La aplicación debe aparecer ahora en la lista de aplicaciones instaladas en HoloLens, lista para su lanzamiento.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-469">Your app should now appear in the list of installed apps on your HoloLens, ready to be launched!</span></span>

    > [!NOTE]
    > <span data-ttu-id="2d7a8-470">Para implementar en auriculares inmersivo, establezca la **plataforma** de la solución en el *equipo local* y establezca la **configuración** en *depurar* , con *x86* como **plataforma** .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-470">To deploy to immersive headset, set the **Solution Platform** to *Local Machine* , and set the **Configuration** to *Debug* , with *x86* as the **Platform** .</span></span> <span data-ttu-id="2d7a8-471">A continuación, implemente en el equipo local, mediante el **menú compilar** , seleccionando *implementar solución* .</span><span class="sxs-lookup"><span data-stu-id="2d7a8-471">Then deploy to the local machine, using the **Build menu** , selecting *Deploy Solution* .</span></span> 

## <a name="chapter-16--using-the-application-on-the-hololens"></a><span data-ttu-id="2d7a8-472">Capítulo 16: uso de la aplicación en HoloLens</span><span class="sxs-lookup"><span data-stu-id="2d7a8-472">Chapter 16 – Using the application on the HoloLens</span></span>

- <span data-ttu-id="2d7a8-473">Una vez que haya iniciado la aplicación, verá el bot como una esfera azul delante de usted.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-473">Once you have launched the application, you will see the Bot as a blue sphere in front of you.</span></span>

- <span data-ttu-id="2d7a8-474">Use el **gesto de TAP** mientras está Gazing en la esfera para iniciar una conversación.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-474">Use the **Tap Gesture** while you are gazing at the sphere to initiate a conversation.</span></span> 
 
- <span data-ttu-id="2d7a8-475">Espere a que se inicie la conversación (la interfaz de usuario mostrará un mensaje cuando se produzca).</span><span class="sxs-lookup"><span data-stu-id="2d7a8-475">Wait for the conversation to start (The UI will display a message when it happens).</span></span> <span data-ttu-id="2d7a8-476">Una vez que reciba el mensaje introductorio del bot, pulse de nuevo en el bot para que se vuelva rojo y empiece a escuchar la voz.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-476">Once you receive the introductory message from the Bot, tap again on the Bot so it will turn red and begin to listen to your voice.</span></span> 

- <span data-ttu-id="2d7a8-477">Una vez que haya dejado de hablar, la aplicación enviará el mensaje al bot y recibirá una respuesta que se mostrará en la interfaz de usuario.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-477">Once you stop talking, your application will send your message to the Bot and you will promptly receive a response that will be displayed in the UI.</span></span> 

- <span data-ttu-id="2d7a8-478">Repita el proceso para enviar más mensajes a su Bot (debe puntear cada vez que desee enviar un mensaje).</span><span class="sxs-lookup"><span data-stu-id="2d7a8-478">Repeat the process to send more messages to your Bot (you have to tap each time you want to sen a message).</span></span>

<span data-ttu-id="2d7a8-479">Esta conversación muestra cómo el bot puede conservar la información (su nombre), a la vez que proporciona información conocida (como los elementos que están almacenados en existencias).</span><span class="sxs-lookup"><span data-stu-id="2d7a8-479">This conversation demonstrates how the Bot can retain information (your name), whilst also providing known information (such as the items that are stocked).</span></span>

#### <a name="some-questions-to-ask-the-bot"></a><span data-ttu-id="2d7a8-480">Algunas preguntas para preguntar al bot:</span><span class="sxs-lookup"><span data-stu-id="2d7a8-480">Some questions to ask the Bot:</span></span>
...
## <a name="your-finished-web-app-bot-v4-application"></a><span data-ttu-id="2d7a8-481">Su aplicación Web App Bot (v4) finalizada</span><span class="sxs-lookup"><span data-stu-id="2d7a8-481">Your finished Web App Bot (v4) application</span></span>

<span data-ttu-id="2d7a8-482">Enhorabuena, ha creado una aplicación de realidad mixta que aprovecha el bot de aplicaciones Web de Azure, Microsoft bot Framework V4.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-482">Congratulations, you built a mixed reality app that leverages the Azure Web App Bot, Microsoft Bot Framework v4.</span></span>

![Producto final](images/AzureLabs-Lab312-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="2d7a8-484">Ejercicios extra</span><span class="sxs-lookup"><span data-stu-id="2d7a8-484">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="2d7a8-485">Ejercicio 1</span><span class="sxs-lookup"><span data-stu-id="2d7a8-485">Exercise 1</span></span>

<span data-ttu-id="2d7a8-486">La estructura de conversación de este laboratorio es muy básica.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-486">The conversation structure in this Lab is very basic.</span></span> <span data-ttu-id="2d7a8-487">Utilice Microsoft LUIS para ofrecer capacidades de comprensión del lenguaje natural de bot.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-487">Use Microsoft LUIS to give your bot natural language understanding capabilities.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="2d7a8-488">Ejercicio 2</span><span class="sxs-lookup"><span data-stu-id="2d7a8-488">Exercise 2</span></span>

<span data-ttu-id="2d7a8-489">En este ejemplo no se incluye la finalización de una conversación y el reinicio de una nueva.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-489">This example does not include terminating a conversation and restarting a new one.</span></span> <span data-ttu-id="2d7a8-490">Para que la característica de bot se complete, intente implementar el cierre de la conversación.</span><span class="sxs-lookup"><span data-stu-id="2d7a8-490">To make the Bot feature complete, try to implement closure to the conversation.</span></span>
