...
> [!NOTE]
> El dictado y el reconocimiento de frases no se pueden controlar al mismo tiempo. Si GrammarRecognizer o KeywordRecognizer están activos, dictationRecognizer no puede estar activo y viceversa.
...
3. En la **sección Publishing Settings > Capabilities (Configuración** de publicación > funcionalidades), compruebe la **funcionalidad Microphone** (Micrófono).
4. Concesión de permisos a la aplicación para el acceso al micrófono en el dispositivo HoloLens
    * Se le pedirá que lo haga al iniciar el dispositivo, pero si accidentalmente hizo clic en "no", puede cambiar los permisos en la configuración del dispositivo.

## <a name="phrase-recognition"></a>Reconocimiento de frases

Para permitir que la aplicación escuche frases específicas habladas por el usuario, debe realizar alguna acción:
...
Por último, empiece a reconocerlo.
...
Cree un `GrammarRecognizer` y pásese la ruta de acceso al archivo SRGS:
...
Las **funcionalidades Cliente** **de** Internet y Micrófono deben declararse para que una aplicación use dictado:
1. En el Editor de Unity, vaya **a Editar > configuración del proyecto > Player.**
2. Seleccione en la **pestaña Tienda Windows.**
3. En la **sección Configuración de publicación > funcionalidades,** compruebe la funcionalidad **InternetClient.**
    * Opcionalmente, si no ha habilitado el micrófono, compruebe la **funcionalidad Micrófono.**
4. Conceda permisos a la aplicación para el acceso al micrófono en el dispositivo HoloLens si aún no lo ha hecho.
...
Si ya no desea mantener dictationRecognizer, debe cancelar la suscripción a los eventos y eliminar dictationRecognizer.
...
* Una vez hecho con el reconocedor, debe eliminarse mediante `Dispose()` para liberar los recursos que usa. Liberará estos recursos automáticamente durante la recolección de elementos no utilizados con un costo de rendimiento adicional si no se liberan antes.
...
También podría iniciar una palabra claveRecognizer, que también reiniciará PhraseRecognitionSystem.
...
Si sigue el recorrido del punto de control de desarrollo de Unity que hemos diseñado, la siguiente tarea es explorar las api y las funcionalidades de Mixed Reality plataforma:
