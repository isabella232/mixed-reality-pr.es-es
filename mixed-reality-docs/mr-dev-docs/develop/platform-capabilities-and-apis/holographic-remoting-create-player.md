...
description: Cree una aplicación personalizada de comunicación remota de Hologafosc para mostrar el contenido representado en un equipo remoto en el HoloLens 2.
...
>[!IMPORTANT]
><a name="idl"></a>Dentro ```build\native\include\HolographicAppRemoting\Microsoft.Holographic.AppRemoting.idl``` del paquete NuGet contiene documentación detallada de la API expuesta por Holographic Remoting.

## <a name="modify-the-packageappxmanifest-of-the-application"></a>Modificar package.appxmanifest de la aplicación

Para que la aplicación sea consciente de los Microsoft.Holographic.AppRemoting.dll agregados por el paquete NuGet, es necesario realizar los pasos siguientes en el proyecto:
...
>[!IMPORTANT]
>Al igual que con cualquier API de C++/WinRT, podría producirse ```Connect``` una excepción winrt::hresult_error que debe controlarse.

La escucha de conexiones entrantes en la aplicación player se puede realizar llamando al ```Listen``` método . Tanto el puerto de protocolo de enlace como el puerto de transporte se pueden especificar durante esta llamada. El puerto de protocolo de enlace se usa para el protocolo de enlace inicial. A continuación, los datos se envían a través del puerto de transporte. De forma predeterminada, se usan los números de puerto **8265** y **8266.**
...
```
2) OnDisconnected: se desencadena si se finaliza una conexión establecida o no se pudo establecer una conexión.
```cpp
...
Además, el estado de conexión se puede consultar mediante la ```ConnectionState``` propiedad en el contexto del reproductor.
```cpp
...
Para mostrar el contenido representado de forma remota, llame ```PlayerContext::BlitRemoteFrame``` a al representar un elemento [HolographicFrame.](/uwp/api/windows.graphics.holographic.holographicframe) 
...
>[!NOTE]
>```PlayerContext::BlitRemoteFrame``` posiblemente sobrescribe el punto de enfoque del marco actual. 
...
```BlitResult``` también puede devolver ```BlitResult::Success_Color_Depth``` en las condiciones siguientes:
...
La propiedad especifica la cantidad de tiempo que se reutiliza un fotograma remoto ```PlayerContext::BlitRemoteFrameTimeout``` si no se recibe ningún fotograma remoto nuevo. 
...
Para diagnosticar problemas de rendimiento o de red, se pueden recuperar estadísticas sobre el último fotograma remoto a través de la ```PlayerContext::LastFrameStatistics``` propiedad . Las estadísticas se actualizan durante la llamada a [HolographicFrame::P resentUsingCurrentPrediction](/uwp/api/windows.graphics.holographic.holographicframe.presentusingcurrentprediction).
...
Para más información, consulte la ```PlayerFrameStatistics``` documentación del ```Microsoft.Holographic.AppRemoting.idl``` [archivo](#idl).
